 ...
 *
 * <p>내 정보는 여기 두지 않는다. 헤더의 아바타가 그 자리를 맡는다 — 사람은 하나뿐이고 늘 같은 것을
 * 여는 자리라, 지도를 오가며 누르는 탭과 같은 줄에 세울 이유가 없다.
 *
 * <p>켜진 자리는 이 띠 안에서 작은 상자로 채우고, 그 상자가 탭 사이를 미끄러져 옮겨 간다. 자리마다 면을
...
export function MobileBottomNav(props: {
  ...
  return (
    <div
      ref={setRow}
      // 헤더와 같은 좌우 여백(12px)에 서고 아래로 24px 띄운다.
      // 두께도 헤더와 같은 46px 다 — 화면 위아래에 같은 굵기의 띠가 하나씩 서면 지도가 그만큼 넓어 보인다.
