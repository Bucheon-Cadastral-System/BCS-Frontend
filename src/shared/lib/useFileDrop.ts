 ...
 *
 * @param options.reject 받지 않는 자리 — 끌어 오는 동안 커서에 금지를 적어 놓기 전에 알린다.
 *   놓았을 때 onFiles 는 그대로 불리므로, 부르는 쪽이 받을 수 없다는 사실만 알리면 된다.
 */
export function useFileDrop(onFiles: (files: File[]) => void, options?: { reject?: boolean }) {
  ...
  const dropHandlers = {
    onDragOver: (e: DragEvent) => {
      ...
      e.stopPropagation()
      if (options?.reject === true) e.dataTransfer.dropEffect = 'none'
      setDragging(true)
    },
