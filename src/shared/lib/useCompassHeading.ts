...
/**
 * 이보다 적게 돌면 새로 그리지 않는다(도).
 *
 * <p>손에 든 기기는 가만히 두어도 1도 안팎으로 떨린다. 그리고 이 값이 바뀔 때마다 지도는 화면 전체를
 * 다시 그리므로(방향 맞추기), 잘게 받을수록 프레임이 깎인다. 4도는 눈으로 방향이 어긋났다고 느끼기 전이면서
 * 떨림과 잔갱신을 함께 걷어 내는 자리다.
 */
const MIN_TURN = 4
