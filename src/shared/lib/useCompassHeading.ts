 ...
 *
 * <p>손에 든 기기는 가만히 두어도 1도 안팎으로 떤다. 그 떨림까지 흘려보내면 서 있기만 해도 지도가 쉬지 않고
 * 다시 그려진다. 크게 잡으면 그 각도만큼씩 끊겨 도는데, 받는 쪽이 매 프레임 따라붙으므로
 * (ControlPointMap 의 GLIDE_MS) 여기서는 떨림만 걸러 내면 된다.
 */
const MIN_TURN = 1
